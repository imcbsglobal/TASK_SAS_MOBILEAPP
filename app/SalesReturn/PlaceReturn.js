...
export default function PlaceOrder() {
  ...
  async function fetchUploadedOrders() {
    ...
    try {
      ...
      // FILTER: Only show returns from the last 2 days
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            if (order.uploadStatus === 'uploaded' || order.uploadStatus === 'uploaded to server') {
              Alert.alert('Already Sync', 'This return entry is already uploaded to the server.');
              return;
            }
            ...
            try {
              ...
              if (uploadResult.success) {
                ...
                const storageKey = `return_orders_${currentUsername || 'unknown'}`;
      ...
      <View style={styles.orderCard}>
        ...
        {isExpanded && (
          <View style={styles.orderBody}>
            ...
            <View style={{ marginTop: 15, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
              <TouchableOpacity
                style={{
                  width: 44,
                  height: 44,
                  borderRadius: 22,
                  backgroundColor: openMenus[order.id] ? Colors.accent.main : Colors.neutral[100],
                  alignItems: 'center',
                  justifyContent: 'center',
                  borderWidth: 1,
                  borderColor: openMenus[order.id] ? Colors.accent.main : Colors.border.light,
                  ...Shadows.sm
                }}
                onPress={() => setOpenMenus(prev => ({ ...prev, [order.id]: !prev[order.id] }))}
              >
                <Ionicons name="share-social-outline" size={22} color={openMenus[order.id] ? "#fff" : Colors.accent.main} />
              </TouchableOpacity>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={styles.header}>
          ...
          <View>
            ...
            <Text style={styles.headerSubtitle}>Manage & Track Returns</Text>
          </View>
          <View style={{ flexDirection: 'row', gap: 10 }}>
            {(filterStatus === 'pending' || filterStatus === 'failed') && (
              ...
              >
                <Ionicons name={selectionMode ? "close" : "checkbox"} size={22} color={Colors.accent.main} />
                <Text style={{ marginLeft: 5, color: Colors.accent.main, fontWeight: '600' }}>
