...
export default function SalesDetails() {
  ...
  const loadDebtors = async () => {
    try {
      ...
      console.error('[SalesDetails] Error loading debtors:', error);
    }
  };
  ...
  const handleSelectCustomer = (selected) => {
    ...
    console.log('[SalesDetails] Changed customer to:', selected.name);
  };
    ...
    const loadCustomerAndCalcPrice = async () => {
      ...
      if ((!currentCustomerObj || currentCustomerObj.code !== currentCustomerCode) && currentCustomerCode) {
        try {
          ...
          console.log('[SalesDetails] Loaded full customer:', currentCustomerObj?.name, 'RemarkTitle:', currentCustomerObj?.remarkcolumntitle);
        } catch (e) {
  ...
  const addToCart = useCallback((product, quantity = 1, startCoords = null, overwrite = false) => {
    ...
    if (idx >= 0) {
      ...
      newCart = [...currentCart, { product, qty: quantity }];
    }
                    ...
                    <>
                      ...
                      <View style={styles.quantityTotalContainer}>
                        ...
                        <Text style={styles.quantityTotalValue}>
                          {((parseFloat(tempQuantity) || 0) * ((appSettings?.order_rate_editable === true || String(appSettings?.order_rate_editable) === 'true') ? (parseFloat(tempPrice) || 0) : selectedProduct.price)).toFixed(3)}
                        </Text>
                      </View>
                          ...
                          <>
                            ...
                            {selectedBatchDetails.price > 0 && !selectedBatchDetails.restrictedCodes?.includes(selectedBatchDetails.priceCodeUsed || 'S2') && (
                              <View style={[styles.priceItem, styles.priceItemHighlight]}>
                                ...
                                <Text style={[styles.priceValue, { color: Colors.success.main }]}>{selectedBatchDetails.price?.toFixed(3)}</Text>
                              </View>
                            )}
    ...
    <View style={styles.productBottomSection}>
      ...
      <View style={styles.actionsContainer}>
        ...
        {currentQty > 0 ? (
          ...
          <View style={{ flexDirection: 'row', alignItems: 'stretch', gap: 4, height: 44, width: '100%' }}>
            {/* Minus Button */}
            <TouchableOpacity
            ...
            <TouchableOpacity
              style={[styles.addButtonLarge, !inStock && styles.disabledAddButton, { flex: 0.5, height: '100%', marginTop: 0 }]}
              onPress={(e) => {
